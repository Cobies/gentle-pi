...
6. **Implement task by task.** Route each task through the orchestrator's Work Routing Ladder, honoring its mandatory delegation triggers, with the configured TDD mode and applicable checks. These triggers are mandatory, not advisory: executing past a fired trigger inline is a routing defect even if the work succeeds. Subagent delegation mandate: all source code mutations, edits, and multi-file work MUST delegate to \`gentle-ai-worker\` and require explicit user permission before dispatch; inline code editing by the parent orchestrator is strictly forbidden. Default to background execution (\`mode: "background"\`) for independent tasks to run concurrently. Every subagent delegation must resolve \`.atl/skill-registry.md\` and inject matching exact paths under \`## Skills to load before work\`, plus \`## Allowed edit surfaces\` for writers. Check an item off only after its outcome and checks were observed; update the file, mirror, and visible \`todo\` projection after every task transition and material plan change. Every task closes with at least one work-unit commit on the feature branch, branch first when on the default branch, with tests and docs alongside the behavior, using a Conventional Commit message; record the commit identity in the feature document as evidence. Work-unit commits on the feature branch are part of authorized substantial ODD implementation; push, pull request creation, and merge remain the user's decisions.
