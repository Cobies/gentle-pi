...
- Linguistic Mapping (HARD CONTRACT): Any user action verb or defect report ('arreglá', 'corregí', 'hacé', 'probá', 'revisá', 'fijate', 'tengo un detalle...') maps directly to subagent delegation. The parent chat is a Pure Thinker and Coordinator; it NEVER performs multi-file edits or heavy codebase sweeps inline.
...
- Strict Role Segregation (Pure Thinker vs. Scout vs. Worker): The parent orchestrator NEVER performs code edits or multi-file sweeps inline. Deep exploration (>=3 reads or cross-file inspection) belongs exclusively to \`gentle-ai-explore\` (or CodeGraph). Code mutations belong exclusively to \`gentle-ai-worker\`. Workers focus strictly on implementing settled tasks against pre-existing specifications, never architectural redesign.
